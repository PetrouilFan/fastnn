...
fn save_model(model: &Bound<'_, PyAny>, path: String) -> PyResult<()> {
    let np = model.call_method0("named_parameters").map_err(|e| {
    ...
    let state_dict: Vec<(String, Tensor)> = params
        .into_iter()
        .map(|(name, pt)| (name, pt.inner))
        .collect();
...
fn load_model(path: String) -> PyResult<HashMap<String, PyTensor>> {
    let state_dict =
        crate::io::serialize::load_model(&path).map_err(|e| IoError::new_err(e.to_string()))?;
    Ok(state_dict
        .into_iter()
