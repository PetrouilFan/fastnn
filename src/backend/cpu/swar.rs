...
//!
//! These kernels compute dot products directly on packed U8x4 / U4x8 words
...
#[inline(always)]
pub fn u8x4_dot_packed_signed(a: u32, b: u32) -> i32 {
...
/// Compute dot product of two U8x4-packed u32 words.
///
/// Each u32 holds 4 × signed i8 values (little-endian byte order, range [-128, 127]).
/// Returns i32 accumulator (exact for K ≤ 8192 without overflow).
#[inline(always)]
pub fn u8x4_dot_packed(a: u32, b: u32) -> i32 {
...
#[inline(always)]
pub fn u4x8_dot_packed(a: u32, b: u32) -> i32 {
    let mut sum = 0i32;
    for i in 0..8 {
        let shift = i * 4;
...
/// Vectorized U8x4 dot product over slices of packed u32 words.
///
/// Computes Σ u8x4_dot_packed(a[i], b[i]) for i in 0..len
/// Auto-vectorizes to SIMD on modern CPUs (AVX2/NEON).
#[inline(always)]
pub fn u8x4_dot_packed_slice(a: &[u32], b: &[u32]) -> i32 {
    debug_assert_eq!(a.len(), b.len());
    let mut sum = 0i32;
    for i in 0..a.len() {
        sum += u8x4_dot_packed(a[i], b[i]);
    }
    sum
}

/// Vectorized U4x8 dot product over slices of packed u32 words.
#[inline(always)]
pub fn u4x8_dot_packed_slice(a: &[u32], b: &[u32]) -> i32 {
    debug_assert_eq!(a.len(), b.len());
    let mut sum = 0i32;
    for i in 0..a.len() {
        sum += u4x8_dot_packed(a[i], b[i]);
    }
    sum
}

/// Quantize FP32 slice to U8x4 packed u32 words (per-tensor asymmetric, signed).
///
/// Returns (packed_words, scale, zero_point)
/// Each output u32 holds 4 quantized signed i8 values (-128..127).
/// Matches U8x4 storage format and from_f32_per_channel_asymmetric.
pub fn quantize_f32_to_u8x4(data: &[f32]) -> (Vec<u32>, f32, f32) {
    if data.is_empty() {
...
/// Quantize FP32 slice to U4x8 packed u32 words (per-tensor asymmetric, signed).
///
/// Returns (packed_words, scale, zero_point)
/// Each output u32 holds 8 quantized signed 4-bit values [-8, 7].
/// Matches U4x8 storage format and from_f32_per_channel_asymmetric.
pub fn quantize_f32_to_u4x8(data: &[f32]) -> (Vec<u32>, f32, f32) {
    if data.is_empty() {
        return (Vec::new(), 1.0, 0.0);
    }
...
/// Dequantize U8x4 packed words to FP32 slice (signed asymmetric).
pub fn dequantize_u8x4_to_f32(packed: &[u32], scale: f32, zero_point: f32, out: &mut [f32]) {
    let mut idx = 0;
    for &word in packed {
        for lane in 0..4 {
            if idx >= out.len() {
...
/// Dequantize U4x8 packed words to FP32 slice (signed asymmetric).
pub fn dequantize_u4x8_to_f32(packed: &[u32], scale: f32, zero_point: f32, out: &mut [f32]) {
    let mut idx = 0;
    for &word in packed {
        for lane in 0..8 {
            if idx >= out.len() {
...
mod tests {
    ...
    #[test]
    fn test_u8x4_dot_packed_basic() {
        let a = 0x04030201u32; // [1, 2, 3, 4]
        let b = 0x01010101u32; // [1, 1, 1, 1]
        assert_eq!(u8x4_dot_packed(a, b), 10);
    }

    #[test]
    fn test_u8x4_dot_packed_zero() {
        assert_eq!(u8x4_dot_packed(0, 0xFFFFFFFF), 0);
        assert_eq!(u8x4_dot_packed(0xFFFFFFFF, 0), 0);
    }
    ...
    #[test]
    fn test_u8x4_dot_packed_signed_example() {
        // [1, 2, 3, 4] = 0x04030201
    ...
    #[test]
    fn test_quantize_dequantize_u8x4_roundtrip() {
