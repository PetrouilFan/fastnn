    ...
    with_wgpu_context(|ctx| -> Result<(), BackendError> {
        ...
        let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry { binding: 1, resource: buf_input.as_entire_binding() },
        ...
        let wgc_x = (h_out * w_out).max(1) as u32;
        let wgc_y = c.max(1) as u32;
        let wgc_z = n.max(1) as u32;
        let mut encoder = ctx
            .device
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (spatial_idx >= params.H_out * params.W_out || c >= params.C || n >= params.N) {
        return;
    }
