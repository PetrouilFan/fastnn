...
// SAFETY: F32x1 is a `repr(transparent)` struct over `u32` (the bit representation of `f32`), so it is safe to
...
impl PackedWord for F32x1 {
    const SCALAR_TYPE: crate::types::ScalarType = crate::types::ScalarType::F32;
    const ITEMS: usize = 1;
