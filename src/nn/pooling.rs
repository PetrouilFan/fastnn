...
impl Module for AvgPool1d {
    fn forward(&self, x: &Tensor) -> Tensor {
        ...
        let result = dispatch(
            "avg_pool2d",
            DispatchKey::Cpu,
            &[&x_4d, &self.kernel_size_scalar, &self.stride_scalar, &self.padding_scalar],
        )
...
impl Module for MaxPool1d {
    fn forward(&self, x: &Tensor) -> Tensor {
        ...
        let result = dispatch(
            "max_pool2d",
            DispatchKey::Cpu,
            &[&x_4d, &self.kernel_size_scalar, &self.stride_scalar, &self.padding_scalar, &self.dilation_scalar],
        )
