...
- Remove unconditional per-op GPU waits from unary, binary, matmul, scalar, and reduction launch paths. Synchronize only on explicit host readback.
...
- Add fused GPU optimizer kernels for SGD, Adam, AdamW, RMSprop, Lion, and Muon update paths.
...
- Visible WGPU unary, binary, scalar, matmul, and supported 2D reduction launch paths no longer poll immediately after submit.
