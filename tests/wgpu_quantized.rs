...
fn test_wgpu_matmul_u4() {
    ...
    assert_eq!(gpu_result.len(), 8, "U4 output should have 8 elements");
    for &v in &gpu_result {
...
fn test_wgpu_matmul_u8() {
    ...
    assert_eq!(gpu_result.len(), 8, "U8 output should have 8 elements");
    for &v in &gpu_result {
...
fn test_wgpu_conv2d_u8() {
    ...
    let cpu_result = run_conv2d(1, 1, 2, 5, 5, 3, 1, 0, &weight, &input, Some(8), CpuBackend);
    let gpu_result = run_conv2d(
        1,
        1,
        2,
