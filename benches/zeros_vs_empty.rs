...
fn main() {
    ...
    println!(
        "{:<20} {:<15} {:<15} {:<10}",
        "Size", "zeros() ms", "empty() ms", "Speedup"
    );
    println!("------------------------------------------------------------");
    ...
    for size in sizes {
        ...
        for _ in 0..100 {
            let _tensor =
                fastnn::Tensor::zeros(size.clone(), fastnn::DType::F32, fastnn::Device::Cpu);
        }
        ...
        for _ in 0..100 {
            let _tensor =
                fastnn::Tensor::empty(size.clone(), fastnn::DType::F32, fastnn::Device::Cpu);
        }
        let empty_time = start.elapsed().as_secs_f64() * 1000.0 / 100.0;

        let speedup = if empty_time > 0.0 {
